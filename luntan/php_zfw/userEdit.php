...
<?php if($select == 1) {
	 ?> 
<table>
<form action="userDao.php?select=1" method="post">
<tr><td>用户名：</td><td><input id="name" name="name"  onblur="namemsg(this)"/><span id="namemsg"></span></td></tr>
...
<?php } else {
	 $userDao = new UserDAO;
	$row = $userDao->findbyid($id);
	
	?>
	
<table>
<form action="userDao.php?select=2&id=<?php echo $id; ?>" method="post">
...
<tr><td>等级：</td><td><select name="degree">
  ...
  <tr><td><input type="submit" value="提交"/></td><td><input value="重置" type="reset"/></td></tr>         
  </form>      
</table>
<?php }?>
</body>
